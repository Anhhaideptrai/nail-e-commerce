...
import { Minus, Plus, ShoppingBag, ChevronDown, ChevronUp, Star, Truck, Package, RotateCcw, ArrowLeft } from 'lucide-react';
  ...
  const accordionSections = [
    {
      ...
      content: (
        <div>
          ...
          <p className="text-[#5A5A5A] text-sm"><span className="text-[#1A1A1A]">Material:</span> {product.material}</p>
          <p className="text-[#5A5A5A] text-sm mt-1"><span className="text-[#1A1A1A]">Processing Time:</span> {product.processingTime}</p>
        </div>
      ),
    },
    {
      ...
      content: (
        <div className="text-sm text-[#5A5A5A] space-y-3">
          ...
          <div className="border border-[#E5E5E5] overflow-hidden">
            <table className="w-full text-xs">
              <thead className="bg-[#F5F5F5]">
                <tr>
                  <th className="px-4 py-2.5 text-left text-[#1A1A1A] uppercase tracking-widest" style={{ letterSpacing: '0.1em' }}>Size</th>
                  <th className="px-4 py-2.5 text-left text-[#1A1A1A] uppercase tracking-widest" style={{ letterSpacing: '0.1em' }}>Nail Width</th>
                  <th className="px-4 py-2.5 text-left text-[#1A1A1A] uppercase tracking-widest" style={{ letterSpacing: '0.1em' }}>Hand Size</th>
                </tr>
              </thead>
          ...
          <p className="text-xs text-[#9A9A9A]">* If ordering Custom Size, please include your measurements in the order notes at checkout.</p>
        </div>
      ),
    },
    {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
          ...
          <div className="md:pt-4">
            {/* Collection */}
            <p className="text-[#9A9A9A] uppercase text-xs tracking-widest mb-2" style={{ letterSpacing: '0.15em' }}>
              {product.collection}
            </p>

            {/* Name */}
            <h1 className="text-[#1A1A1A] mb-4" style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 400, fontSize: 'clamp(1.6rem, 4vw, 2.2rem)', lineHeight: 1.2 }}>
              {product.name}
            </h1>
            ...
            <div className="flex items-center gap-2 mb-5">
              ...
              <span className="text-[#9A9A9A] text-xs">{product.rating} ({product.reviewCount} reviews)</span>
            </div>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <p className="text-[#1A1A1A] text-xs uppercase tracking-widest" style={{ letterSpacing: '0.12em' }}>
            ...
            {/* Shape Selector */}
            <div className="mb-6">
              <p className="text-[#1A1A1A] text-xs uppercase tracking-widest mb-3" style={{ letterSpacing: '0.12em' }}>
            ...
            <div className="flex gap-3 mb-4">
              <div className="flex items-center border border-[#E0E0E0]">
                ...
                <span className="px-4 text-sm text-[#1A1A1A] min-w-[2.5rem] text-center">{quantity}</span>
                <button
            ...
            {!selectedSize && <p className="text-[#C0C0C0] text-xs mb-6">Please select size, shape, and length</p>}
            ...
            <div className="space-y-0">
              {accordionSections.map(section => (
                <div key={section.key} className="border-t border-[#F0F0F0]">
                  ...
                  >
                    {section.title}
                    {openSection === section.key ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
                  </button>
        ...
        <section className="border-t border-[#F0F0F0] py-20 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-12">
              <p className="text-[#9A9A9A] uppercase tracking-[0.2em] text-xs mb-3" style={{ letterSpacing: '0.2em' }}>You May Also Like</p>
              <h2 className="text-[#1A1A1A]" style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 400, fontSize: 'clamp(1.5rem, 3vw, 2rem)' }}>
                Related Products
              </h2>
            </div>
